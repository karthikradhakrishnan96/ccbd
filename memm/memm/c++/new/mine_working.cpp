...
void get_features(struct dataset x,std::string y,double* a)
{
	...
	if ((determiners.find(current_word) != determiners.end() ) && y == "DT") a[8] = 1;
	...
	if ( (current_word == "there" && existance.find(next_word) != existance.end()) && y == "EX" )
		 a[9] =1;

    // Personal pronoun: i,we,she etc
	
	if ((personal_pronns.find(current_word) != personal_pronns.end() ) && y == "PRP") a[10] = 1;

	// A feature for TO 
	// note that current_word has been transformed to lowercase a lil while before
	if (current_word == "to" && y == "TO" ) a[11] = 1;
	...
	if ( (modal_verb.find(current_word) !=  modal_verb.end())  && y == "MD") a[13] = 1;
...
void memm()
{
	...
	std::cout<<global_dataset.size()<<std::endl;
